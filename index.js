  ...
  sock.ev.on('messages.upsert', async ({messages}) => {
    ...
    switch(text[0]){
      ...
      case 'Bub':
        ...
        }else await sock.sendMessage(jid, {text: 'My bubub cuman ' + `@${OWNER_PHONE_NUMBER}` + ' seorang'}, {quoted: msg}, {mentions: [`${OWNER_PHONE_NUMBER}@s.whatsapp.net`]})
        break
      case 'kapan':
      case 'Kapan':
        if(userId == `${OWNER_PHONE_NUMBER}@s.whatsapp.net`){
          await sock.sendMessage(jid, {text: 'Kapan kapan ya bub💕'}, {quoted: msg})
        }else await sock.sendMessage(jid, {text: 'Kapan kapan'}, {quoted: msg})
        break
      case '.whenyah':
      case 'when':
      case 'When':
        ...
        else await sock.sendMessage(jid, {text: 'When when'}, {quoted: msg})
        break
      case 'test' :
