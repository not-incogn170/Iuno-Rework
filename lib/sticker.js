...
async function fromVideo(sock, jid, msg, downloadMediaMessage) {
    ...
    try{
        ...
        ffmpeg(bufferStream)
            ...
            .outputOptions([
                "-vcodec", "libwebp",
                "-vf", "scale=512:512:force_original_aspect_ratio=decrease,fps=12,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=#00000000",
                "-qscale:v", "20",
                "-lossless", "0",
                "-compression_level", "6",
                "-loop", "0"
...
async function toimage(sock, jid, msg, downloadMediaMessage) {
    const tmpDir = os.tmpdir()
    const tempFile = path.join(tmpDir, `image_${Date.now()}_${Math.floor(Math.random() * 1000)}.png`)
