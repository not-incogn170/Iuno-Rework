...
async function fromReply(sock, jid, msg, downloadMediaMessage) {
    const quotedImage = msg.quoted?.message?.imageMessage
    const quotedVideo = msg.quoted?.message?.videoMessage

    try{
        if(quotedImage){
            await fromImage(sock, jid, msg, downloadMediaMessage)
        }else if(quotedVideo){
            await fromVideo(sock, jid, msg, downloadMediaMessage)
        }else{
            await sock.sendMessage(jid, {text: "Please reply to an image or video message to create a sticker."}, {quoted: msg})
        }
    }catch(e){
        await sock.sendMessage(jid, {text: String(e)}, {quoted: msg})
    }
}
module.exports = {
    fromImage,
